#!/bin/bash
#
## File: actlisthostapps.sh
## Lists applications associated to a host on an Actifio appliance from Linux or MacOS commmand line
...
printf "Hostname: %-32s : HostID: %-8s\n" \$ahost \$host_id
reportapps -nca \$host_id | while IFS="," read -r -a chost ; do
printf "AppType: %-33s : AppName: %-20s : AppID: %-8s\n" \${chost[0]} \${chost[2]} \${chost[3]}
done
printf "\n"
done
exit
